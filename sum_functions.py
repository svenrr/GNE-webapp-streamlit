...
def get_summary(article_text, limit_percent=0.1):
    ...
    * limit_percent = Percentages of sentences to keep (i.e. 0.1 for 10%)
    ...
    st.write("Readability: {}".format(str(textstat.text_standard(article_text, float_output=False)))  #st.write(str(textstat.text_standard(article_text, float_output=False))) #+ st.write("([Flesch Reading Ease:](https://en.wikipedia.org/wiki/Flesch%E2%80%93Kincaid_readability_tests#Flesch_reading_ease) ") + st.write(str(textstat.flesch_reading_ease(article_text))) + st.write(")")
